...
@semiformal decorated function
  -> semi(f"semantic prompt with {variables}")
    ...
    -> resolver.resolve(portal, usage, fingerprint, constants)
      ...
      -> ADVANCE / GENERATE: build GenerationSpec with DAG context (parent_sources, lineage_summary)
...
| `resolver.py` | resolve(portal, usage, fingerprint, constants) -> REUSE / ADVANCE / GENERATE (ResolutionResult) |
...
| `generator.py` | OpenAI API wrapper; system + user prompt construction |
| `validator.py` | 3-stage validation: AST parse, type check, execution test with sample input |
| `agent.py` | Generate-validate-retry loop; uses GenerationSpec.decision and parent_sources for ADVANCE/FORK prompts |
| `console_io.py` | Rich-based console output; DAG-aware logs (print_dag_reuse, print_dag_advance, print_dag_generate); print_slot_history (git-log-style) |
...
- **Decision**: REUSE, ADVANCE, FORK, GENERATE, MERGE (resolution outcome)
...
- Resolution: refs[usage_id] -> REUSE; else operation_signature match -> REUSE (and add ref); else branch with same fingerprint -> ADVANCE; else GENERATE (new branch or new slot)
- Logging: DAG-aware one-line logs (slot:branch -> DECISION -> commit id); print_slot_history(slot) for git-log-style history
